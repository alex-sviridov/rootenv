...
|---|---|---|---|
| `RELAY_EXEC_IMAGE` | yes | — | Image for the relay-primitive container. Reconcile returns error if missing. |
...
| `RELAY_EXEC_INGRESS_BASE_PATH` | no | `/relay` | Path prefix. Full path = `<base>/<envName>`. |
...
### Ingress
- Path: `<RELAY_EXEC_INGRESS_BASE_PATH>/<envName>`, `PathType: Prefix`
